...
class Solver():
    ...
    def solve_a_star(self):
        ...
        while nodes.qsize() and epochs <= self.__max_iter:
            ...
            if cur_state == self.__goal_state.flatten().tolist():
                self.__summary = str("A* took " + str(cur_node.get_level()) + " steps to get from initial state to the desired goal, visited total of " + str(epochs) + " nodes, and took around " + str(np.round(time.time() - start_time, 4)) + " seconds to reach the desired solution.")
    ...
    def solve_bfs(self):
        ...
        while nodes.qsize() and epochs <= self.__max_iter:
            ...
            if cur_state == self.__goal_state.flatten().tolist():
                self.__summary = str("BFS took " + str(cur_node.get_level()) + " steps to get from initial state to the desired goal, visited total of " + str(epochs) + " nodes, and took around " + str(np.round(time.time() - start_time, 4)) + " seconds to reach the desired solution.")
                while cur_node.get_parent():
                    self.__path.append(cur_node)
                    cur_node = cur_node.get_parent()
                break
